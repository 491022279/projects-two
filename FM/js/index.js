...
window.onload=function(){
    ...
    let tiaol=document.querySelector('.tiaol');
    let tiaor=document.querySelector('.tiaor');
    let tiaos=document.querySelector('.tiaos');
    let audio=document.querySelector('audio');
    let play=document.querySelector('.bofang');
    let pause=document.querySelector('.zanting');
    let previous=document.querySelector('.previous');
    let next=document.querySelector('.next');
    let shengyin=document.querySelector('.shengyin');
    let jingyin=document.querySelector('.jingyin');
    let volume=document.querySelector('.volume');
    let volume0=document.querySelector('.volume0');
    ...
    play.onclick=function(){
        if(audio.paused){
            audio.play();
            pause.classList.toggle('bofang');
        }else{
            audio.pause();
            play.classList.toggle('bofang');
        }
    }
    ...
    volume.onmousedown=function(){
        volume.onmousemove=function(e){
            let oy= Math.abs(volume.offsetHeight-e.offsetY);
            let bili=oy/volume.offsetHeight;
            console.log(oy)
            volume0.style.height=oy+'%';
            audio.volume=bili;
        }
    ...
    audio.ontimeupdate=function(){
        ...
        let string='';
        tiaol.innerText=current;
        tiaor.innerText=duration;
        tiaos.style.width=audio.currentTime/audio.duration*100+'%';
    ...
    function render(obj){
        ...
        audio.src=obj.src;
        p1.innerText=`${obj.songs}`;
        p2.innerText=`${obj.name}`;
        img.src=obj.photo;
        tiaol.innerText='00:00';
        tiaor.innerText=obj.alltime;
